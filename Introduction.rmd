...
The progression from $S$ to $I$ depends on: prevalence of infected, prevalence of susceptibles, the underlying population contact structure, and the probability of transmission given contact. These factors are described by the parameter $\beta$: the rate of a **successful infectious contact**.
...
$\beta SI$: The **rate of new infections** among individuals at risk. It depends from how many risk contact an average person has $(\beta)$, adjusted by the ratio of infectious individuals among these contacts $(I)$, and by the rate of susceptible individuals $(S)$ still available.
...
.blockquote[To stop the epidemic a population must reach a **level of immunity** such that:

$$R \ge 1 - \frac{1}{\mathcal{R}_0}$$
]
...
A typical example are STI with no immunity. Let's assume a high risk group $H$ and a low risk group $L$ and no lifelong immunity (SIS model). Considering that $\frac{dS}{dt} = -\frac{dI}{dt}$ the model becomes:
...
And the general $\mathcal{R}_0$? it depends from the dynamic in the **slaved phase**. Its computation requires more high level algebra to compute $I_H^s,I_L^s$, i.e., the distribution of $I$ among groups in the slaved phase. In this case we have:
...
.blockquote[
Each risk group drives an $\mathcal{R}_0$ which depends on the **group relative proportions** and **infectiousness**. The general $\mathcal{R}_0$ depends on the **infection proportion between groups in the slaved phase** and lies between the group $\mathcal{R}_0$.
]
...
.blockquote[
- The small high risk group drives the high $\mathcal{R}_0$ but the large high risk group keep the group prevalences down.
