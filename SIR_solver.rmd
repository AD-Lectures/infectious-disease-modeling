...
Than lets declare the epidemic parameters to get an R_0 = 2:
...
`r kable(head(sir_values))`

Use `ggplot` to create a SIR visualization function

```{r}
plot_sir <- function(SIR) {
	SIR %>%
    # transform data into "long format"
    tidyr::pivot_longer(-time, names_to = 'Comp', values_to = 'Value') %>%
    # plot three lines, with a differnt color for each comparment
    ggplot(aes(time, Value, color = Comp)) +
    geom_line() +
    # plot formatting
    theme_minimal() +
    labs(x = "Time", y = "Population fraction", color = "Compartment")
}
```
...
In this case we still use a large R_0 = 2, so the herd immunity threshold is 50%. What happens if the fraction of immunes is below the threshold?
