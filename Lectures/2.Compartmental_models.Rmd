---
title: "Introduction to Infectious Disease Modeling - Compartmental Models"
output:
...
```{r, include=FALSE}
plot_folder <- file.path("plots", "compartmental-models/")
...
.fig75[
```{r contact rate, echo = FALSE, fig.width=10, fig.height=2.5}

slope <- .5
ggplot() +
  geom_hline(size = 1, aes(yintercept = 20, color = "Freq-based")) +
  geom_abline(size = 1, aes(intercept = 0, slope = slope, color = "Dens-based")) +
  coord_fixed(ratio = .5, xlim = c(0, 100), ylim = c(0, 50)) +
  scale_y_continuous(breaks = c(0, 20, 75), labels = c("", "k", "")) +
  annotate("text", x = 75, y = slope * 75 + slope * 10, label = "k = v * A/N", angle = atan(slope * .5) * 180 / pi, size = 5) +
  annotate("text", x = 75, y = slope * 75 + slope * 10, label = "k = v * A/N", angle = atan(slope * .5) * 180 / pi, size = 5) +
  theme_minimal() +
  theme(axis.text.x = element_blank()) +
  labs(x = "Population density A/N", y = "Contact rate k", color = "Model type")
```
]
---
...
risk_plot <- lapply(c(1, 2, 5, 10, 100), function(k) {
  tibble(p = seq(0, 1, length.out = 100), k, q = 1 - (1 - p)^k)
}) %>%
  bind_rows() %>%
  ggplot(aes(p, q, color = reorder(paste("kI/N =", k), k))) +
  geom_line() +
  theme_minimal() +
